...
import { getReminderByDedupKey, markReminderFailed } from "@/lib/reminder-contract";
...
export const POST: APIRoute = async (context) => {
  ...
  const payload = (await context.request.json().catch(() => null)) as MarkFailedPayload | null;
  ...
  if (!licenseRef || !recipientEmail || !reminderDate || !errorMessage) {
    return new Response(
      JSON.stringify({ error: "licenseRef, recipientEmail, reminderDate, and errorMessage are required" }),
      { status: 400 },
    );
  }
  ...
  const markResult = await markReminderFailed(supabase, reminderResult.data, errorMessage);
  if (markResult.error) {
