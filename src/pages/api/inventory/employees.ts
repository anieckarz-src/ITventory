...
import { upsertEmployee } from "@/lib/inventory-records";
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    const { error } = await upsertEmployee(supabase, companyId, form);
    ...
    return dashboardRedirect(context, "error", error instanceof Error ? error.message : "Employee save failed");
  }
};
